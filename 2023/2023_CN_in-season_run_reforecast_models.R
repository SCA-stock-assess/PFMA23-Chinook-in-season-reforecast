...
pkgs <- c("tidyverse","readxl","ggrepel","bbmle","ggpmisc", "ggmap", 
          "reshape2","glmmTMB","lme4","DHARMa","MuMIn", "sf", "broom")
...
# Load interview data
cpue <- read_xlsx("CN_return_predictors.xlsx",
                  sheet = "CREEL Interview Summary 2023") |> 
  rename_with(tolower) |> 
  ...
  mutate(statsub = case_when( # Combine subareas that were split in 2011
    ...
    TRUE ~ statsub)) |>  
  group_by(year, month, statsub, sw_2023, prop_rch) |> 
  ...
  filter(sw_2023 %in% c(82, 83, 84, 91, 92)) |> # Keep only the mid-Aug to mid-Sept stat weeks
  pivot_longer(cols = cn_all_k:boat_trips) |> 
  pivot_wider(names_from = sw_2023,
              values_from = value) |> 
  rowwise() |> 
...
# Save basemap
library(ggmap)
Barkley <- get_stamenmap(bbox = c(left = -125.587, bottom = 48.753, right = -124.708, top = 49.145), # Bounding box
                         maptype = "terrain", 
                         zoom = 11)
