...
tmp <- set_names(subarea_combos, 
  ...
  map(~ filter(cpue_minimal, statsub %in% .x) |> 
        group_by(year, return, period) |>                          # drop prop_rch
        summarize(across(c(cn_all_k, rch_cn_k, boat_trips), \(x) sum(x, na.rm = TRUE)),
                  .groups = "drop") |>                              # explicit column list, not a range
        mutate(cpue = cn_all_k / boat_trips,
               rch_cpue = rch_cn_k / boat_trips) |> 
...
cpue_long <- cpue |> 
  ...
  pivot_longer(cols = contains("cpue"), names_to = "predictor", values_to = "x") |> 
  pivot_longer(cols = contains("return"), names_to = "response", values_to = "y") |> 
  mutate(group = paste(statsub, period, predictor, response, sep = "-")) |> 
  group_by(group) |> 
  filter(!(all(is.na(x) | is.infinite(x)) | all(is.na(y) | is.infinite(y))))


cpue_lms <- split(cpue_long, cpue_long$group) |> 
  map(~ lm(y ~ x, data = filter(.x, !is.infinite(x), !is.infinite(y))))
...
# What percentile of the observed range does the current year value correspond to?
bs_cn |> 
